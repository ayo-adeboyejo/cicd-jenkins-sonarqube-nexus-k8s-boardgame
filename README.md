...
A CI/CD pipeline for a Spring Boot application. It implements the full DevOps lifecycle from source code to a running Kubernetes deployment. Every stage is defined as code in a Jenkinsfile, version-controlled alongside the application, and reproducible from a single `git push`.
...
- Integrating **SonarQube** for static analysis across Java, HTML and JavaScript 
- Publishing build artefacts to **Nexus Repository Manager** using Maven deployment
...
# Add Trivy repository to apt source
...
> **Jenkins Docker group membership:** The Docker daemon by default binds to the unix socket (runs only as root). Jenkins must be in the `docker` group to send commands to this socket without using sudo.
...
# Generates the default configuration for containerd, save it and edit it.
...
# Installs and locks the packages to prevent them from being automatically upgraded
# when you run a standard system update 
...
The SonarQube server must be registered in Jenkins before `withSonarQubeEnv()` can be used in the pipeline. This tells Jenkins where the SonarQube server is and which credentials to use when communicating with it.


1. Go to **Manage Jenkins → Configure System** to configure integrations with external services.
2. Scroll to the **SonarQube servers** section
3. Check **Environment variables** — enables `withSonarQubeEnv()` in the pipeline
...
The **Config File Provider Plugin** manages `settings.xml` inside Jenkins and are used to provide Nexus credentials:
...
pipeline {
        ...
        stage('SonarQube Analysis') {
            steps {
                withSonarQubeEnv('sonarqube') {
                    ...
                    '''
                    // sonar.sources includes resources so HTML/JS are
                    // analysed for quality but coverage.exclusions prevents
                    // them from being counted in the coverage metric
                }
            }
        }
        ...
        stage('Quality Gate') {
            steps {
                timeout(time: 1, unit: 'HOURS') {
                    // polls SonarQube API until analysis completes
                    // abortPipeline: false — reports result without blocking deploy
        ...
        stage('Publish Artefact to Nexus') {
            steps {
                withMaven(
                    ...
                    jdk: 'jdk25',
                    maven: 'maven3',
                    traceability: true    // links build number to artefact in Nexus
                ) {
                    sh 'mvn deploy'
                }
            }
    ...
    post {
        always {
            archiveArtifacts artifacts: 'reports/trivy-fs-report.html, \
                reports/trivy-image-report.html', fingerprint: true
...
Both reports are archived as Jenkins build artefacts and available for download from the build page.

The `reports/` directory is explicitly separated from source code to prevent SonarQube from treating Trivy HTML output as analysable source files.
...
# Stage 2 — runtime (contains JRE only, no build tools, no source code)
...
The runtime image contains only what is needed to run the application — the JRE, the JAR, and a non-root user. The JDK, Maven, and source code from the build stage are discarded. This minimises the attack surface and reduces the image size significantly.
...
### Lombok annotation processing not configured

Even after upgrading Lombok, `@Data`-generated getters were missing at compile time. The root cause was that `maven-compiler-plugin` was not configured with `annotationProcessorPaths` — without this, Maven does not run Lombok as an annotation processor during compilation and `@Data`, `@Getter`, `@AllArgsConstructor` produce no output. Adding the `annotationProcessorPaths` block with the correct Lombok version resolved all fifteen `cannot find symbol` errors simultaneously.
